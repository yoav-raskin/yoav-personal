...
data_allocations <- read_sheet("https://docs.google.com/spreadsheets/d/1roHpcMq16zLkxzE5iXozFZQ8DmCWYwYdiCKTraKVD_U/edit#gid=0") %>%
  mutate(MonthYear = paste0(Month, "-", Year),
         DateTemp = myd(paste0(MonthYear, "-", "01")),
         DateStr = as.character(DateTemp))

data_expanses <- read_sheet("https://docs.google.com/spreadsheets/d/13Sm9igrD7Q8PotXwkxYUObgkon_V3TuFtngLX53BHMw/edit#gid=28022007") %>%
  select(-Timestamp, -`I gave the receipt to Masha!`, -`Is reimbursement required?`) %>% 
...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "Holon Budget App"),
